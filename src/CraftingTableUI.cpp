...
CraftingTableUI::CraftingTableUI(PlayerInventory& playerInventory, Texture& inventoryAtlas, Texture& itemAtlas)
...
void CraftingTableUI::updateGUI()
{
    setCraftingTableUI();
    setItemDecals(0.0f, 0.0f);
}

void CraftingTableUI::render()
{
    vao.Bind();
    texture.Bind();
...
void CraftingTableUI::renderInventoryText()
{
    ...
    float craftingStartX = inventoryBottomLeftX + 30.0f * scale;
    float craftingStartY = inventoryBottomLeftY + 17.0f * scale;
    ...
    float craftingResultStartX = inventoryBottomLeftX + 124.0f * scale;
    float craftingResultStartY = inventoryBottomLeftY + 35.0f * scale;
...
void CraftingTableUI::addText(float posX, float posY, int quantity)
{
...
void CraftingTableUI::playerClick(float glX, float glY, bool* itemSelected, bool leftClick)
{
...
void CraftingTableUI::interactLeftClick(Item** slots, int slotCol, int slotRow, int numCols)
{
...
void CraftingTableUI::interactRightClick(Item** slots, int slotCol, int slotRow, int numCols)
{
    int index = slotCol + slotRow * numCols;
    if (*itemSelected == false && slots[index] != nullptr) {
        *itemSelected = true;
...
void CraftingTableUI::interactCraftResult(Item** slots, int slotCol, int slotRow)
{
    if (craftingResult != nullptr)
    {
        Crafting::completeCraft<3>(craftingStorage, craftingResult, selectedItem, itemSelected);
    }
}

void CraftingTableUI::updateInventoryPosition() {
    float inventoryWidth = 175.0f * scale;
    float inventoryHeight = 164.0f * scale;
...
SlotRegion CraftingTableUI::getClickedSection(float glX, float glY) 
{  
    float slotTransformX;
    float slotTransformY;
...
bool CraftingTableUI::checkRegion(int slotCol, int slotRow, float numCols, float numRows, float rangeX, float rangeY, bool rowOffset) {
...
void CraftingTableUI::checkCraft() {

    Item* newCraftingResult = Crafting::canCraft<3>(craftingStorage);
...
void CraftingTableUI::setCraftingTableUI()
{
    float centerX = screenWidth / 2.0f;
    float centerY = screenHeight / 2.0f;
...
void CraftingTableUI::setItemDecals(float glX, float glY)
{
    mouseX = glX;
    mouseY = glY;
    ...
    inventoryStartX = inventoryBottomLeftX + 30.0f * scale;
    ...
    inventoryStartX = inventoryBottomLeftX + 124.0f * scale;
